...
# Detect available container runtimes on system
detect-runtimes:
    @echo "Detecting container runtimes..."
    @which nerdctl > /dev/null && echo "✓ nerdctl found" || echo "✗ nerdctl not found"
    @which podman > /dev/null && echo "✓ podman found" || echo "✗ podman not found"
    @which docker > /dev/null && echo "✓ docker found" || echo "✗ docker not found"

# Show runtime versions
runtime-versions:
    @echo "Container runtime versions:"
    @which nerdctl > /dev/null && nerdctl --version || echo "nerdctl: not installed"
    @which podman > /dev/null && podman --version || echo "podman: not installed"
